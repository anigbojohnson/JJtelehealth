...
$.ajaxSetup({
        ...
        $('#submit').click(function() {
            ...
            console.log()
            fetch("/submit-carer-leave-certificate", {
              method: 'POST',
              headers: {
          ...
          .then(data => {
              window.location.href = `/payment/${encodeURIComponent(data.id)}/${encodeURIComponent('MC')}/${encodeURIComponent("carer-Leave-certificate")}`;
          ...
          document.getElementById('back-carer').addEventListener('click', function() {
            $('#carerDetails').show();   
  ...
  formData.delete('careForSomeone')
  ...
  if ($('#careForSomeone').val()=="default") {
    $('#careForSomeone-error').text(errorMessage);
    isValid = false;
} else {
  formData.set('careForSomeone', $('#careForSomeone').val());
  formDataObject = Object.fromEntries(formData);

    $('#careForSomeone-error').text('');
    document.getElementById('previewCareForSomeone').innerText = formDataObject.careForSomeone
    if($('#careForSomeone').val()=="Yes"){
        if (!$('#personCared').val()) {
            $('#personCared-error').text(errorMessage);
            isValid = false;
        }else{
            formData.set('personCared', $('#personCared').val());
            formDataObject = Object.fromEntries(formData);
            $('#personCared-error').text('');
            document.getElementById('previewPersonCared').innerText = formDataObject.personCared

        }
        $("#previewPersonCaredRow").show()
    }else{
        $("#previewPersonCaredRow").hide()
    }
...
$('#carer-button').click(function() {
    var isValid = true;
    var errorMessage = "This field is required";
  
    formData.delete('careForSomeone')
    formData.delete('personCared')
    ...
    if (validTo) {
        ...
        if (validToDate < validFromDate) {
            ...
            if (diffDays > 3) {
                $('#validTo-error').text("The valid period must not exceed 14 days.");
                isValid = false;
            } else{
...
$(document).ready(function() {
    $('#careForSomeone').change(function() {
        if ($(this).val() === 'Yes') {
            
            $('#personCaredFor').show('d-none');
        } else {
            $('#personCaredFor').hide('d-none');
        }
    });

    // Trigger change event on page load to handle initial state
    $('#careForSomeone').trigger('change');
});
